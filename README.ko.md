...
- **Jev 비용:** 변속 기록 위에 기록된 Jev 호출의 총비용, 호출 수, 입력 토큰, 호출당 평균을 보여 주고, 각 턴 줄과 게이트 판에는 그 턴의 비용이 나옵니다. Jev는 비용이 아니라 토큰 수만 돌려주기 때문에, 기록된 토큰에 정가(`scripts/jev_dashboard.py`의 `JEV_PRICE_PER_MTOK`: 입력 100만 토큰당 $0.042, 출력 무료, 2026-09-23 확인)를 곱해 계산합니다. Jev를 호출하지 않은 fallback 턴은 $0입니다.
...
프록시를 지나는 모델 요청마다 모델 이름, 실제로 보낸 effort, CLI가 원래 요청한 effort를 기록합니다(프롬프트 내용은 기록하지 않음). 그래서 턴마다 어떤 모델이 어떤 effort 레벨로 실행됐는지 정확히 보여 줍니다. 이 기록이 생기기 전의 요청은 "not observed"로 나옵니다.
