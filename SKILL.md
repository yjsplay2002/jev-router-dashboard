...
No host accepts an effort parameter from a hook, so the numeric level is applied by the user: Claude and Grok `/effort <level>`, Codex `Alt+.` / `Alt+,` or `/model`. The hook prints the exact command; do not claim the level was applied unless the user applied it. With `"auto_apply_effort": true` in config.json the hook instead writes a level Jev routed into the host's settings (Claude `effortLevel` in `settings.json`, Codex top-level `model_reasoning_effort` in `config.toml`). That changes the default the host reads later, not the running turn: Codex reads it at the next session, and Claude only if it rereads settings mid-session. Say so rather than claiming the current turn ran at that level. Codex has no skill-scoped effort override yet (openai/codex#22908) and Claude's `effort:` frontmatter is reported as having no runtime effect (anthropics/claude-code#69267); when either lands, the same hook can invoke a per-level skill instead of printing a command.
