...
Echo whichever line you get verbatim before you act. Past the cap, or on any Jev server error, nothing is routed and the user's own level applies (the provider's default gear if set, otherwise the CLI's effort); Jev being slow or unreachable is a normal outcome, not an error to report as a failure.
